...
@app.route('/search')
def search_results():
    query_string, tag_query = request.args.get('query', '').strip(), request.args.get('tag', '').strip().lower()
    page, per_page = request.args.get('page', 1, type=int), 10
    search_filter, results_title = {}, "Search Results"
    if query_string:
        results_title = f"Search Results for: \"{query_string}\""; regex_query = re.compile(query_string, re.IGNORECASE)
        search_filter['$or'] = [{'title': {'$regex': regex_query}}, {'body': {'$regex': regex_query}}]
    elif tag_query: results_title = f"Questions tagged with: \"{tag_query}\""; search_filter['tags'] = tag_query
    else: flash("Please enter a search term or select a tag.", "info"); return redirect(url_for('index'))
    questions_cursor = mongo.db.questions.find(search_filter).sort('timestamp', -1).skip((page - 1) * per_page).limit(per_page)
    questions_with_details = []
    for q_doc in questions_cursor:
        author = mongo.db.users.find_one({'_id': q_doc['author_id']})
        q_doc.update({'author_username': author['username'] if author else 'Unknown', 'answers_count': mongo.db.answers.count_documents({'question_id': q_doc['_id']}), 'net_votes': len(q_doc.get('upvotes', [])) - len(q_doc.get('downvotes', []))})
        questions_with_details.append(q_doc)
    total_questions = mongo.db.questions.count_documents(search_filter)
    total_pages = (total_questions + per_page - 1) // per_page
    return render_template('search_results.html', title=results_title, questions=questions_with_details, query=(query_string or tag_query), page=page, total_pages=total_pages, total_results=total_questions)
...
if __name__ == '__main__':
    import logging
    logging.basicConfig(level=logging.INFO) # Basic logging for INFO and above
    # Example: For more detailed Flask logs in development
